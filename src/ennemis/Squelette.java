...
public class Squelette extends Ennemi {
    ...
    private static final Image ennemiImages = Mondes.Ressources.spriteSheet_Ennemis.getSubImage(3, 1).getScaledCopy(2);

    /**
     * Class constructor
     */
    public Squelette(int x, int y, Taille taille, Direction direction, int timer, float vitesse, Image ennemiImages) {
        super("Squelette", x, y, taille.getLargeur(), taille.getLongeur(),
                50, direction, timer, vitesse, ennemiImages);
        this.chargerImage();
    }
    
    public Squelette(Point positon, Taille taille, Direction direction, int timer, float vitesse , Image ennemiImages) {
        this(positon.getX(), positon.getY(), taille, direction, timer, vitesse, ennemiImages);
    }

    public Squelette(Point p, Direction direction) {
        this(p.getX(), p.getY(), Squelette.SIZE, direction, Squelette.TIMER, Squelette.VITESSE, Squelette.ennemiImages);
    }
